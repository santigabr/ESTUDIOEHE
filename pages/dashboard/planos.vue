...
const { data: planos, refresh } = await useFetch('/api/planos')
...
async function add() {
  errors.value = {
    ...
    desc: desc.value === '' ? 'Coloca una descripcion' : '',
    images: images.value.length === 0 ? 'Coloca al menos una imagen' : '',
    files: files.value.length === 0 ? 'Coloca al menos un pdf' : '',
    price: price.value === '' ? 'Coloca un precio' : '',
  }
...
<template>
  <div class="max-w-1200px mx-auto px-6 pt-6">
    ...
    <div class="pt-5 grid md:grid-cols-3 gap-3 sm:grid-cols-2 grid-cols-1">
  ...
  <Modal v-if="addmenu">
    ...
    <Counter :value="bathrooms" label="Baños" @update:value="garages = $event" />
    <Counter :value="rooms" label="Habitaciones" @update:value="garages = $event" />
    <Counter :value="garages" label="Garages" @update:value="garages = $event" />
    <Counter :value="suites" label="Suites" @update:value="garages = $event" />
